...
export const pow = (a: BN.Value, b: BN.Value): string => new BN(a).pow(b).toString();

export const sum = (array: BN.Value[]): string => (array.length ? BN.sum.apply(null, array.filter(isFinite)).toString() : "0");

export const min = (array: BN.Value[]): string => BN.min.apply(null, array.filter(isFinite)).toString();

export const max = (array: BN.Value[]): string => BN.max.apply(null, array.filter(isFinite)).toString();
...
export const isFinite = (n?: BN.Value): boolean => n != null && new BN(n).isFinite();

export const isInteger = (n?: BN.Value): boolean => n != null && new BN(n).isInteger();

export const floorSixDecimal = (input: BN.Value): string => new BN(input).decimalPlaces(MATH.DIGIT, BN.ROUND_FLOOR).toString();

export const roundSixDecimal = (input: BN.Value): string => new BN(input).decimalPlaces(MATH.DIGIT, BN.ROUND_HALF_UP).toString();

export const floor18Decimal = (input: BN.Value): string => new BN(input).decimalPlaces(18, BN.ROUND_FLOOR).toString();
